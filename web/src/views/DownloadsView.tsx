...
export function DownloadsView() {
    ...
    const fetchDownloads = () => {
        fetch('/api/downloads')
            .then(res => res.json())
            .then(data => setDownloads(data?.transfers || {}))
        ...
        <div className="flex flex-col h-full animate-in fade-in duration-300">
            <header className="mb-4 flex justify-between items-end border-b border-bbs-panel pb-2">
                <div>
                    ...
                    <p className="text-bbs-muted text-sm mt-1">HTTP Origin Ingestion</p>
                </div>
                    ...
                    return (
                        <div key={dl.file_id} className="border border-bbs-panel bg-bbs-surface/50 p-4 rounded-sm relative group">
                            ...
                            <div className="mt-4 space-y-2 relative z-10">
                                {dl.error && (
                                    <div className="text-bbs-red text-xs mb-2 font-mono uppercase bg-red-900/20 p-2 rounded-sm border border-red-900/50">
                                        Error: {dl.error}
                                    </div>
                                )}
                                <div className="flex justify-between text-xs text-bbs-muted relative">
                                    <span className="flex items-center space-x-1">
                                        <HardDrive size={12} /> <span>Chunks: {dl.downloaded_chunks} / {dl.total_chunks > 0 ? dl.total_chunks : '?'} {dl.total_chunks > 0 && `(${Math.round((dl.downloaded_chunks / dl.total_chunks) * 100)}%)`}</span>
                                    </span>
                                        ...
                                        <div className="absolute right-0 top-6 w-64 bg-black border border-bbs-cyan text-bbs-text p-3 rounded-sm z-50 shadow-[0_0_15px_var(--color-bbs-cyan-dim)]">
                                            ...
                                            <ul className="space-y-1 max-h-40 overflow-y-auto custom-scrollbar font-mono text-[10px]">
                                                {Array.isArray(dl.peers) && dl.peers.length > 0 ? (
                                                    dl.peers.map((p, i) => (
                                                        <li key={i} className="truncate text-bbs-muted hover:text-white pointer-events-none" title={p}>{p}</li>
                                                    ))
                                                ) : (
